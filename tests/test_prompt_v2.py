...
from src.extraction.vertex_client import (
    init_client, generate_content_safe, 
    detect_supplier, load_suppliers_csv,
    read_excel_safe
)
...
def post_process_promotions(order: ExtractedOrder) -> ExtractedOrder:
    ...
    for barcode, items in grouped_items.items():
        ...
        print(f"  🔄 Promotion detected: avg price {avg_net_price:.2f} for {len(items)} lines of {barcode} (Total Qty: {total_qty})")
...
def run_full_cycle(file_path, email_body=""):
    ...
    prompt_text = get_invoice_extraction_prompt(
        email_context=email_body if email_body else None,
    ...
    response = generate_content_safe(
        ...
        contents=[
            types.Content(
                role="user",
                parts=[file_part, types.Part.from_text(text=prompt_text)]
            )
        ],
    ...
    for i, order in enumerate(orders):
        ...
        for item in order.line_items:
            ...
            print(f"  {desc:<35} | {item.quantity:>5} | {item.raw_unit_price:>10.2f} | {item.discount_percentage:>5.1f}% | {item.final_net_price:>10.4f}")
