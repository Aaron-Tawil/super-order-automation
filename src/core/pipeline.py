...
class ExtractionPipeline:
    ...
    def run_pipeline(
        ...
        detected_code, confidence, detection_method = self.local_detector.detect_supplier(
            file_path=file_path, mime_type=mime_type, email_metadata=email_metadata
        )
        ...
        if detected_code != "UNKNOWN":
            ...
            detected_code, confidence, phase1_cost, reasoning, raw_data_p1, detected_email, detected_id = (
                detect_supplier(
        ...
        if result.supplier_code != "UNKNOWN":
            ...
            if detected_email:
                ...
                if "@" in detected_email and not self.local_detector._is_blacklisted_email(detected_email):
                    logger.info(
                        f"{trace_ctx}🧠 Auto-Learning: Attempting to link {detected_email} to {result.supplier_code}"
                    )
                    try:
                        success, was_added = self.supplier_service.add_email_to_supplier(
                            result.supplier_code, detected_email
                        )
                        if success and was_added:
                            logger.info(
                                f"{trace_ctx}🎉 Auto-Learned: {detected_email} is now linked to {result.supplier_code}"
                            )
            ...
            if detected_id:
                logger.info(
                    f"{trace_ctx}🧠 Auto-Learning: Attempting to link ID {detected_id} to {result.supplier_code}"
                )
                try:
                    success, was_added = self.supplier_service.update_missing_global_id(
                        result.supplier_code, detected_id
                    )
                    if success and was_added:
                        logger.info(
                            f"{trace_ctx}🎉 Auto-Learned: ID {detected_id} is now linked to {result.supplier_code}"
                        )
                    elif success and not was_added:
        ...
        for i, order in enumerate(result.orders):
            ...
            if result.supplier_code == "UNKNOWN" or result.confidence < 0.7:
                ...
                if matched != "UNKNOWN":
                    ...
                    logger.info(
                        f"{trace_ctx}✅ Supplier Fallback Match: {result.supplier_name} ({result.supplier_code})"
                    )
                ...
                if new_barcodes:
                    ...
                    for item in new_items:
                        if item.barcode not in seen:
                            ...
                            items_to_add.append(
                                {"barcode": item.barcode, "name": item.description, "item_code": item.barcode}
                            )
