...
def show_supplier_table(suppliers: list, search: str = "", key_version: int = 0):
    ...
    event = st.dataframe(df, width="stretch", hide_index=True, on_select="rerun", selection_mode="single-row", key=key)
...
def show_edit_form(supplier_service: SupplierService, supplier_code: str, suppliers: list):
    ...
    with st.form(key=f"edit_form_{supplier_code}"):
        ...
        with col1:
            ...
            name = st.text_input(get_text("lbl_name"), value=supplier.get("name", ""), key=f"edit_name_{supplier_code}")
        ...
        with col2:
            email = st.text_input(
                get_text("lbl_email"), value=supplier.get("email", ""), key=f"edit_email_{supplier_code}"
            )
            phone = st.text_input(
...
def show_add_form(supplier_service: SupplierService):
    ...
    with st.form(key="add_form"):
        ...
        with col1:
            code = st.text_input(get_text("lbl_code"), key="add_code", placeholder=get_text("ph_code"))
            name = st.text_input(get_text("lbl_name"), key="add_name", placeholder=get_text("ph_name"))
            global_id = st.text_input(
                get_text("lbl_global_id") + " *", key="add_global_id", placeholder=get_text("ph_global_id")
            )

        with col2:
            email = st.text_input(get_text("lbl_email"), key="add_email", placeholder=get_text("ph_email"))
            phone = st.text_input(get_text("lbl_phone"), key="add_phone", placeholder=get_text("ph_phone"))
...
def main():
    ...
    with form_col:
        if st.session_state.get("selected_supplier"):
            show_edit_form(supplier_service, st.session_state["selected_supplier"], suppliers)
        else:
